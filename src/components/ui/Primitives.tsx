...
/* ---------- country code tile ---------- */
export function CC({ cc, size = 40 }: { cc: any, size?: number }) {
  return (
...
/* ---------- countdown ring ---------- */
export function CountdownRing({ progress, size = 168, label, sub, color = "var(--accent)" }: { progress: number, size?: number, label?: React.ReactNode, sub?: React.ReactNode, color?: string }) {
  const stroke = 6;
...
/* ---------- toast system ---------- */
export type Toast = { id?: string; msg: string; kind?: "ok" | "bad" | "info"; icon?: string; dur?: number };
